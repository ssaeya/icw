...
public class OrderPayloadTypeHandler extends BaseTypeHandler<Object>{
...
//		return cps;
	...
	@Override
	public OrderPayload getNullableResult(ResultSet rs, int columnIndex) throws SQLException {
		...
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			cp = objectMapper.readValue(d.toString(), OrderPayload.class);
		} catch (JsonParseException e) {
			e.printStackTrace();
		} catch (JsonMappingException e) {
			e.printStackTrace();
	...
	@Override
	public OrderPayload getNullableResult(CallableStatement cs, int columnIndex) throws SQLException {
